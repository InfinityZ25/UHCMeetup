package me.infinity.minigame.meetup.Database.Core;
    ...
    public static void connect() {
        if (!isConnected()) {
            try {
                ...
                Meetup.getInstance().log(" Mysql has been successfully established!");
            } catch (SQLException e) {
                Meetup.getInstance().log(" Mysql connection could not be established!");
                Bukkit.getConsoleSender().sendMessage("jdbc:mysql://" + MySQL.host + ":" + MySQL.port + "/" + MySQL.database);
                Bukkit.getServer().getPluginManager().disablePlugin(Meetup.getInstance());
                e.printStackTrace();
            }
        }
    }
